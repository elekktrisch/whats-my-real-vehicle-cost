  ...
  template: `
    <section class="hero-card">
      <div class="hero-grid">
        ...
        <!-- Money → asset arrow. The SVG itself stays the same; CSS rules
             on .hero-arrow shrink it as compact-progress climbs. On
             mobile (single column) it rotates 90° (downward); the
             rotation toggles via media query, not progress. -->
...
export class HeroSummary {
  /** Compaction is now driven by `--compact-progress` — a CSS variable set
   * on the comparison-page sticky wrapper and read by `.hero-card` rules
   * in styles.css. The component itself doesn't take a compact input. */
