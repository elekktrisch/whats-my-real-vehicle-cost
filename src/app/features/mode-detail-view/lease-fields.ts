...
/**
 * Lease-specific controls. Opportunity-cost rate moved to globals (it's
 * cross-mode); lease-end-section stays here because every fee inside it is
 * lease-only. Advanced-tier fees inside lease-end-section auto-hide based
 * on `store.basicMode()` — handled inside that molecule.
 */
@Component({
  selector: 'app-lease-fields',
  imports: [SliderControl, SliderGroup, LeaseEndSection],
  template: `
    <div class="flex flex-col gap-[14px]" role="tabpanel" id="modepanel-lease" aria-labelledby="modetab-lease">
